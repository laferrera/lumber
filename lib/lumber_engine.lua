...
Lumber.FileSelect = require "fileselect"

local SCREEN_FRAMERATE = 15

Lumber.sample_changed_callback = function() end
Lumber.meta_changed_callback = function() end
Lumber.waveform_changed_callback = function() end
Lumber.play_positions_changed_callback = function() end
Lumber.views_changed_callback = function() end

Lumber.setup_params_dirty = false
Lumber.filter_dirty = false
Lumber.env_dirty = false
Lumber.lfo_functions_dirty = false
Lumber.lfo_1_dirty = false
Lumber.lfo_2_dirty = false
Lumber.bpm = 120
Lumber.display = "id" -- Can be "id", "note" or "none"
Lumber.shift_mode = false
Lumber.file_select_active = false
...
Lumber.specs = specs
Lumber.options = options
Lumber.samples_meta = samples_meta
Lumber.num_sample_params = 0
...
local function update_freq_multiplier(sample_id)
  ...
  elseif scale_by == 3 then
    multiplier = sample_duration / (options.BY_BARS_DECIMAL[params:get("by_bars_" .. sample_id)] * (60 / Lumber.bpm * 4))
  end
...
local function update_by_bar_multipliers()
  for i = 0, Lumber.num_sample_params - 1 do
...
function Lumber.load_sample(id, file)
...
local function set_marker(id, param_prefix)
  ...
  Lumber.views_changed_callback(id)
end
...
local function sample_loaded(id, streaming, num_frames, num_channels, sample_rate)
  ...
  Lumber.sample_changed_callback(id)
  Lumber.meta_changed_callback(id)
  Lumber.waveform_changed_callback(id)
  Lumber.play_positions_changed_callback(id)
...
function Lumber.clear_samples(first, last)
  first = first or 0
  last = last or first
  ...
  for i = first, last do
    ...
    Lumber.meta_changed_callback(i)    
    Lumber.waveform_changed_callback(i)
    Lumber.play_positions_changed_callback(i)
  end
  
  Lumber.views_changed_callback(nil)
  Lumber.setup_params_dirty = true
end

function Lumber.request_waveform(sample_id)
...
local function move_copy_update(from_id, to_id)
  ...
  Lumber.setup_params_dirty = true
  Lumber.filter_dirty = true
  Lumber.env_dirty = true
  Lumber.lfo_functions_dirty = true
  Lumber.views_changed_callback(nil)
end

function Lumber.move_sample(from_id, to_id)
...
function Lumber.copy_sample(from_id, to_first_id, to_last_id)
...
function Lumber.copy_params(from_id, to_first_id, to_last_id, param_ids)
...
function Lumber.osc_event(path, args, from)
...
osc.event = Lumber.osc_event
-- NOTE: If you need the OSC callback in your script then Lumber.osc_event(path, args, from)
-- must be called from the end of that function to pass the data down to this lib

function Lumber.set_bpm(bpm)
  Lumber.bpm = bpm
...
local function format_hide_for_stream(sample_id, param_name, formatter)
  return function(param)
    if Lumber.samples_meta[sample_id].streaming == 1 then
      return "N/A"
    else
      if formatter then
...
function Lumber.add_params()
  
  params:add_separator("Lumber")
  
  params:add{type = "trigger", id = "clear_all", name = "Clear All", action = function(value)
    Lumber.clear_samples(0, #samples_meta - 1)
  end}
  params:add{type = "control", id = "global_cutoff_freq", name = "Global Cutoff Freq", controlspec = specs.FILTER_FREQ, formatter = Formatters.format_freq, action = function(value)
     for sampleID=0,#samples_meta - 1 do
      engine.filterFreq(sampleID, value)
    end
...
function Lumber.add_sample_params(id, include_beat_params, extra_params)
  ...
  Lumber.num_sample_params = Lumber.num_sample_params + 1
end
...
Lumber.UI = {}
Lumber.UI.__index = Lumber.UI

function Lumber.draw_title(sample_id, show_sample_name)
  if show_sample_name == nil then show_sample_name = true end
  
  screen.level(15)
  
  if Lumber.shift_mode then
    screen.rect(0, 4, 1, 5)
    screen.fill()
  end
  ...
  screen.move(4, 9)
  if Lumber.display == "id" then
    screen.text(string.format("%03d", sample_id))
    screen.move(23, 9)
  elseif Lumber.display == "note" then
    screen.text(MusicUtil.note_num_to_name(sample_id, true))
    screen.move(27, 9)
    max_title_width = 96
  end
  
  if show_sample_name or Lumber.shift_mode then
    local title
...
Lumber.UI.SampleSetup = {}
Lumber.UI.SampleSetup.__index = Lumber.UI.SampleSetup
...
local function update_setup_params(self)
  ...
  Lumber.setup_params_dirty = false
  Lumber.views_changed_callback(self.sample_id)
end

function Lumber.UI.SampleSetup.new(sample_id, index)
  ...
  setmetatable(Lumber.UI.SampleSetup, {__index = Lumber.UI})
  setmetatable(sample_setup, Lumber.UI.SampleSetup)
...
function Lumber.UI.SampleSetup:set_sample_id(id)
  self.sample_id = id
  Lumber.setup_params_dirty = true
...
function Lumber.UI.SampleSetup:set_index(index)
...
function Lumber.UI.SampleSetup:set_param_default()
...
function Lumber.UI.SampleSetup:set_param_delta(delta)
  if self.selected_param_name ~= "" then
...
function Lumber.UI.SampleSetup:enc(n, delta)
  
  if self.move_active then
    if n == 2 or n == 3 then
      self.move_to = util.clamp(util.round(self.move_to + delta), 0, Lumber.num_sample_params - 1)
    end
    
  elseif self.copy_active or self.copy_params_active then
    if n == 2 then
      self.copy_to_first = util.clamp(util.round(self.copy_to_first + delta), 0, Lumber.num_sample_params - 1)
      self.copy_to_last = util.clamp(self.copy_to_last, self.copy_to_first, Lumber.num_sample_params - 1)
    elseif n == 3 then
      self.copy_to_last = util.clamp(util.round(self.copy_to_last + delta), self.copy_to_first, Lumber.num_sample_params - 1)
    end
...
function Lumber.UI.SampleSetup:key(n, z)
  if z == 1 then
    ...
    if self.move_active then
      ...
      elseif n == 3 then
        Lumber.move_sample(self.sample_id, self.move_to)
        self.move_active = false
        self.move_to = 0
        Lumber.views_changed_callback(self.sample_id)
      end
    ...
    elseif self.copy_active then
      ...
      elseif n == 3 then
        Lumber.copy_sample(self.sample_id, self.copy_to_first, self.copy_to_last)
        self.copy_active = false
        self.copy_to_first = 0
        self.copy_to_last = 0
        Lumber.views_changed_callback(self.sample_id)
    ...
    elseif self.copy_params_active then
      ...
      elseif n == 3 then
        Lumber.copy_params(self.sample_id, self.copy_to_first, self.copy_to_last, {})
        self.copy_params_active = false
        self.copy_to_first = 0
        self.copy_to_last = 0
        Lumber.views_changed_callback(self.sample_id)
      ...
      if n == 3 then
        ...
        if self.index == 1 then
          Lumber.file_select_active = true
          Lumber.FileSelect.enter(_path.audio, function(file)
            Lumber.file_select_active = false
            Lumber.views_changed_callback(self.sample_id)
            if file ~= "cancel" then
              Lumber.load_sample(self.sample_id, file)
            end
          end)
...
function Lumber.UI.SampleSetup:sample_key(sample_id)
  
  if self.move_active then
    Lumber.move_sample(self.sample_id, sample_id)
    self.move_active = false
    self.move_to = 0
    Lumber.views_changed_callback(self.sample_id)
    
  elseif self.copy_active then
    Lumber.copy_sample(self.sample_id, sample_id, sample_id)
    self.copy_active = false
    self.copy_to_first = 0
    self.copy_to_last = 0
    Lumber.views_changed_callback(self.sample_id)
  ...
  elseif self.copy_params_active then
    Lumber.copy_params(self.sample_id, sample_id, sample_id)
    self.copy_active = false
    self.copy_to_first = 0
    self.copy_to_last = 0
    Lumber.views_changed_callback(self.sample_id)
...
function Lumber.UI.SampleSetup:redraw()
  
  if Lumber.setup_params_dirty then
    update_setup_params(self)
    self.selected_param_name = self.param_names[self.index]
  end
  
  Lumber.draw_title(self.sample_id)
  local info = ""
...
Lumber.UI.Waveform = {}
Lumber.UI.Waveform.__index = Lumber.UI.Waveform

function Lumber.UI.Waveform.new(sample_id)
  local waveform = {
    sample_id = sample_id or 1,
    tab_id = 1,
  ...
  setmetatable(Lumber.UI.Waveform, {__index = Lumber.UI})
  setmetatable(waveform, Lumber.UI.Waveform)
  return waveform
end

function Lumber.UI.Waveform:set_sample_id(id)
  self.sample_id = id
  Lumber.views_changed_callback(id)
end

function Lumber.UI.Waveform:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
end

function Lumber.UI.Waveform:enc(n, delta)
...
function Lumber.UI.Waveform:key(n, z)
  if z == 1 then
    if n == 2 then
    ...
    elseif n == 3 then
      ...
      if not samples_meta[self.sample_id].waveform_requested then
        Lumber.request_waveform(self.sample_id)
...
function Lumber.UI.Waveform:update()
...
function Lumber.UI.Waveform:redraw()
  local X = 4
  local Y = 25
  local W = 120
  local H = 35
...
Lumber.UI.FilterAmp = {}
Lumber.UI.FilterAmp.__index = Lumber.UI.FilterAmp
...
function Lumber.UI.FilterAmp.new(sample_id, tab_id)
  ...
  setmetatable(Lumber.UI.FilterAmp, {__index = Lumber.UI})
  setmetatable(filter_amp, Lumber.UI.FilterAmp)
  return filter_amp
end

function Lumber.UI.FilterAmp:set_sample_id(id)
  self.sample_id = id
  Lumber.filter_dirty = true
  Lumber.views_changed_callback(id)
end

function Lumber.UI.FilterAmp:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
...
function Lumber.UI.FilterAmp:enc(n, delta)
  if Lumber.shift_mode then delta = delta * 0.1 end
  if self.tab_id == 1 then
    if n == 2 then
...
function Lumber.UI.FilterAmp:key(n, z)
  if z == 1 then
    if n == 2 then
      self:set_tab(self.tab_id % 2 + 1)
    elseif n == 3 then
      if self.tab_id == 1 then
        params:set("filter_type_" .. self.sample_id, params:get("filter_type_" .. self.sample_id) % #Lumber.options.FILTER_TYPE + 1)
      end
    end
    Lumber.views_changed_callback(self.sample_id)
  end
...
function Lumber.UI.FilterAmp:redraw()
  
  Lumber.draw_title(self.sample_id)
  
  if Lumber.filter_dirty then
...
Lumber.UI.Env = {}
Lumber.UI.Env.__index = Lumber.UI.Env

function Lumber.UI.Env.new(env_name, sample_id, tab_id)
  ...
  setmetatable(Lumber.UI.Env, {__index = Lumber.UI})
  setmetatable(env, Lumber.UI.Env)
  return env
end

function Lumber.UI.Env:set_sample_id(id)
  self.sample_id = id
  Lumber.env_dirty = true
  Lumber.views_changed_callback(id)
end

function Lumber.UI.Env:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
end

function Lumber.UI.Env:enc(n, delta)
  
end

function Lumber.UI.Env:key(n, z)
  if z == 1 then
    if n == 2 then
      self:set_tab(self.tab_id % 2 + 1)
    end
...
Lumber.UI.AmpEnv = {}
Lumber.UI.AmpEnv.__index = Lumber.UI.AmpEnv

function Lumber.UI.AmpEnv.new(sample_id, tab_id)
  local env = Lumber.UI.Env.new("amp", sample_id, tab_id)
  setmetatable(Lumber.UI.AmpEnv, {__index = Lumber.UI.Env})
  setmetatable(env, Lumber.UI.AmpEnv)
  return env
end

function Lumber.UI.AmpEnv:enc(n, delta)
  if Lumber.shift_mode then delta = delta * 0.1 end
  if self.tab_id == 1 then
    if n == 2 then
...
Lumber.UI.ModEnv = {}
Lumber.UI.ModEnv.__index = Lumber.UI.ModEnv

function Lumber.UI.ModEnv.new(sample_id, tab_id)
  local env = Lumber.UI.Env.new("mod", sample_id, tab_id)
  setmetatable(Lumber.UI.ModEnv, {__index = Lumber.UI.Env})
  setmetatable(env, Lumber.UI.ModEnv)
  return env
end

function Lumber.UI.ModEnv:enc(n, delta)
  if Lumber.shift_mode then delta = delta * 0.1 end
  if self.tab_id == 1 then
    if n == 2 then
...
Lumber.UI.Lfos = {}
Lumber.UI.Lfos.__index = Lumber.UI.Lfos
...
local function generate_lfo_wave(sample_id, lfo_id)
  ...
  return function(x)
    ...
    if fade > 0 then
      fade_end = util.linlin(0, Lumber.specs.LFO_FADE.maxval, 0, 1, fade)
      y_fade = util.linlin(0, fade_end, MIN_Y, 1, x)
    else
      fade_end = util.linlin(Lumber.specs.LFO_FADE.minval, 0, 0, 1, fade)
      y_fade = util.linlin(0, fade_end, 1, util.linlin(Lumber.specs.LFO_FADE.minval * 0.2, 0, MIN_Y, 1, fade), x)
    end
    
    x = x * util.linlin(Lumber.specs.LFO_1_FREQ.minval, Lumber.specs.LFO_1_FREQ.maxval, 0.5, 10, freq)
    local y
...
function Lumber.UI.Lfos.new(sample_id, tab_id)
  ...
  setmetatable(Lumber.UI.Lfos, {__index = Lumber.UI})
  setmetatable(lfos, Lumber.UI.Lfos)
  return lfos
end

function Lumber.UI.Lfos:set_sample_id(id)
  self.sample_id = id
  Lumber.lfo_functions_dirty = true
  Lumber.views_changed_callback(id)
end

function Lumber.UI.Lfos:set_tab(id)
  self.tab_id = util.clamp(id, 1, 2)
  self.lfo_1_graph:set_active(self.tab_id == 1)
  self.lfo_2_graph:set_active(self.tab_id == 2)
end

function Lumber.UI.Lfos:enc(n, delta)
  if Lumber.shift_mode then delta = delta * 0.05 end
  if self.tab_id == 1 then
    if n == 2 then
...
function Lumber.UI.Lfos:key(n, z)
  if z == 1 then
    if n == 2 then
      self:set_tab(self.tab_id % 2 + 1)
    elseif n == 3 then
      if self.tab_id == 1 then
        params:set("lfo_1_wave_shape", params:get("lfo_1_wave_shape") % #Lumber.options.LFO_WAVE_SHAPE + 1)
      else
        params:set("lfo_2_wave_shape", params:get("lfo_2_wave_shape") % #Lumber.options.LFO_WAVE_SHAPE + 1)
      end
    end
    Lumber.views_changed_callback(self.sample_id)
  end
...
function Lumber.UI.Lfos:update()
...
function Lumber.UI.Lfos:redraw()
  
  Lumber.draw_title(self.sample_id)
  
  if Lumber.lfo_functions_dirty then
  ...
  if Lumber.lfo_1_dirty then
    self.lfo_1_graph:update_functions()
    Lumber.lfo_1_dirty = false
  end
  if Lumber.lfo_2_dirty then
...
Lumber.UI.ModMatrix = {}
Lumber.UI.ModMatrix.__index = Lumber.UI.ModMatrix

function Lumber.UI.ModMatrix.new(sample_id, index)
  local matrix = {
    sample_id = sample_id or 1,
    index = index or 1
  }
  setmetatable(Lumber.UI.ModMatrix, {__index = Lumber.UI})
  setmetatable(matrix, Lumber.UI.ModMatrix)
  return matrix
end

function Lumber.UI.ModMatrix:set_sample_id(id)
  self.sample_id = id
  Lumber.views_changed_callback(id)
end

function Lumber.UI.ModMatrix:set_index(index)
  self.index = util.clamp(index, 1, 11)
end

function Lumber.UI.ModMatrix:enc(n, delta)
  if n == 2 then
    self:set_index(self.index + delta)
  elseif n == 3 then
    if Lumber.shift_mode then delta = delta * 0.1 end
...
function Lumber.UI.ModMatrix:key(n, z)
  if n == 3 and z == 1 then
    if self.index == 1 then
...
function Lumber.UI.ModMatrix:redraw()
  
  Lumber.draw_title(self.sample_id, false)
...
Lumber.UI.KeyMatrix = {}
Lumber.UI.KeyMatrix.__index = Lumber.UI.KeyMatrix

function Lumber.UI.KeyMatrix.new(sample_id, index)
  local matrix = {
    sample_id = sample_id or 1,
    index = index or 1
  }
  setmetatable(Lumber.UI.KeyMatrix, {__index = Lumber.UI})
  setmetatable(matrix, Lumber.UI.KeyMatrix)
  return matrix
end

function Lumber.UI.KeyMatrix:set_sample_id(id)
  self.sample_id = id
  Lumber.views_changed_callback(id)
end

function Lumber.UI.KeyMatrix:set_index(index)
  self.index = util.clamp(index, 1, 3)
end

function Lumber.UI.KeyMatrix:enc(n, delta)
  if n == 2 then
    self:set_index(self.index + delta)
  elseif n == 3 then
    if Lumber.shift_mode then delta = delta * 0.1 end
...
function Lumber.UI.KeyMatrix:key(n, z)
  if n == 3 and z == 1 then
    if self.index == 1 then
...
function Lumber.UI.KeyMatrix:redraw()
