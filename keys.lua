...
local function note_on(voice_id, freq, vel, sample_id)
  ...
  engine.noteOn(voice_id, MusicUtil.note_num_to_freq(60), vel, sample_id)
...
function GlobalView:redraw()
  ...
  for i = 0, NUM_SAMPLES - 1 do
    ...
    if Lumber.samples_meta[i] and Lumber.samples_meta[i].num_frames > 0 then
      if Lumber.samples_meta[i].playing then screen.level(15) else screen.level(3) end
      screen.text(i)
    else
      screen.level(1)
      screen.text("/")
    ...
    if Lumber.samples_meta[i].playing then
      for k, v in pairs(Lumber.samples_meta[i].positions) do
        local note = k % 128
...
function redraw()
  ...
  if Lumber.file_select_active then
...
function init()
  ...
  Lumber.sample_changed_callback = function(id)
    ...
    if Lumber.samples_meta[id].manual_load then
      
      -- Set our own loop point defaults
      params:set("loop_start_frame_" .. id, util.round(Lumber.samples_meta[id].num_frames * 0.2))
      params:set("loop_end_frame_" .. id, util.round(Lumber.samples_meta[id].num_frames * 0.4))
  ...
  Lumber.meta_changed_callback = callback_set_screen_dirty
  ...
  for i = 0, NUM_SAMPLES - 1 do
    Lumber.add_sample_params(i)
  end
  
  -- Default sample
  Lumber.load_sample(0, _path.code .. "/lumber/audio/piano-c.wav")
  
  -- UI
  
  global_view = GlobalView.new()
  sample_setup_view = Lumber.UI.SampleSetup.new(current_sample_id)
  ...
  key_matrix_view = Lumber.UI.KeyMatrix.new(current_sample_id)
