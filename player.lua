...
local Lumber = include("lumber/lib/lumber_engine")
...
local function load_folder(file, add)
  ...
  if add then
    for i = NUM_SAMPLES - 1, 0, -1 do
      if Lumber.samples_meta[i].num_frames > 0 then
        sample_id = i + 1
        break
      end
    end
  ...
  Lumber.clear_samples(sample_id, NUM_SAMPLES - 1)
  ...
  local found = false
  for k, v in ipairs(Lumber.FileSelect.list) do
    ...
    if found then
      ...
      if string.find(lower_v, ".wav") or string.find(lower_v, ".aif") or string.find(lower_v, ".aiff") or string.find(lower_v, ".ogg") then
        Lumber.load_sample(sample_id, folder .. v)
...
local function note_on(sample_id, vel)
  if Lumber.samples_meta[sample_id].num_frames > 0 then
...
local function clear_queue()
  ...
  for k, v in pairs(note_queue) do
    if Lumber.samples_meta[v.sample_id].playing then
    ...
    for i = #note_queue, 1, -1 do
      if note_queue[i].sample_id == sample_id then
        if note_queue[i].event_type ~= event_type then
          ...
          if Lumber.samples_meta[sample_id].playing then
            sample_status[sample_id] = STATUS.PLAYING
          else
...
function grid_redraw()
  ...
  for i = 1, num_leds do
    ...
    elseif sample_status[i - 1] == STATUS.STARTING or sample_status[i - 1] == STATUS.PLAYING then
      leds[i] = 15
    elseif Lumber.samples_meta[i - 1].num_frames > 0 then
      leds[i] = 4
    end
  end
...
function GlobalView:redraw()
  ...
  for i = 1, num_to_draw do
    if Lumber.samples_meta[i - 1].num_frames > 0 then
      draw_grid = true
      break
    end
  end
...
function redraw()
  ...
  if file_select_active or Lumber.file_select_active then
    Lumber.FileSelect.redraw()
    return
  end
...
function init()
  ...
  Lumber.set_bpm(beat_clock.bpm)
  
  -- Lumber callbacks
  Lumber.sample_changed_callback = function(id)
    
    -- Set loop default based on sample length or name
    if Lumber.samples_meta[id].manual_load and Lumber.samples_meta[id].streaming == 0 and Lumber.samples_meta[id].num_frames / Lumber.samples_meta[id].sample_rate < 1 and string.find(string.lower(params:get("sample_" .. id)), "loop") == nil then
  ...
  Lumber.meta_changed_callback = function(id)
    if Lumber.samples_meta[id].playing and sample_status[id] ~= STATUS.STOPPING then
      sample_status[id] = STATUS.PLAYING
    elseif not Lumber.samples_meta[id].playing and sample_status[id] ~= STATUS.STARTING then
      sample_status[id] = STATUS.STOPPED
    end
    grid_dirty = true
    callback_set_screen_dirty(id)
  end
  Lumber.waveform_changed_callback = callback_set_waveform_dirty
  Lumber.play_positions_changed_callback = callback_set_waveform_dirty
  Lumber.views_changed_callback = callback_set_screen_dirty
  ...
  params:add{type = "option", id = "display", name = "Display", options = {"IDs", "Notes"}, default = 1, action = function(value)
    if value == 1 then Lumber.display = "id"
    else Lumber.display = "note" end
  end}
  ...
  for i = 0, NUM_SAMPLES - 1 do
        ...
        if value == 1 then
          for n = #note_queue, 1, -1 do
            if note_queue[n].sample_id == i then
              ...
              if Lumber.samples_meta[i].playing then
    ...
    Lumber.add_sample_params(i, true, extra_params)
  end
  ...
  sample_setup_view = Lumber.UI.SampleSetup.new(current_sample_id, nil)
  waveform_view = Lumber.UI.Waveform.new(current_sample_id)
  filter_amp_view = Lumber.UI.FilterAmp.new(current_sample_id)
  amp_env_view = Lumber.UI.AmpEnv.new(current_sample_id)
  mod_env_view = Lumber.UI.ModEnv.new(current_sample_id)
  lfos_view = Lumber.UI.Lfos.new(current_sample_id)
  mod_matrix_view = Lumber.UI.ModMatrix.new(current_sample_id)
